...
//                未完成时 toast 拦截, 避免 KYC 未过加卡导致数据脏
//                kycCompleted 由 MKHomeViewController 拉 /user/suphome 后写入(userStatus != 10), 持久化在 NSUserDefaults
...
- (void)addTapped {
    ...
    // kycCompleted 由 MKHomeViewController 拉 /user/suphome 后写入 (userStatus != 10), 跨 VC 共享
    if (![MKLoginManager sharedManager].kycCompleted) {
